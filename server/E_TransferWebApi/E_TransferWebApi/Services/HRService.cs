...
namespace E_TransferWebApi.Services
{
    public interface IHRService
    {
        ...
        //E-mail methods
        bool EmailForApprovalInHr(int id, RequestDetails request);

        bool EmailRejected(string id, RequestDetails req);
    }
    ...
    public class HRService : IHRService
    {
        ...
        public bool EmailForApprovalInHr(int id, RequestDetails request)
        {
            ...
            foreach (AssetDetails del in detaillist)
            {
                if (del.EmployeeCode == request.EmployeeCode)
                {
                    ...
                    bodyBuilder.HtmlBody =
                        @"<div> Dear Sir/Madam </div><br><br><div>An Asset Acceptance request has been send to you. Please take the necessary actions.</div><br><div> Hr Department</div>";
        ...
        public bool EmailRejected(string id, RequestDetails req)
        {
            ...
            if (temp.Supervisor == req.SupervisorCode)
            {
                ...
                bodyBuilder.HtmlBody =
                    @"<div>  Dear Supervisor </div><br><br><div>Your Request has been rejected by Hr for the stated reason: " +
                    id + "</div><br><div> Hr Department</div>";
                message.Body = bodyBuilder.ToMessageBody();
